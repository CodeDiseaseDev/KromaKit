...
static char g_nvg_last_error[256] = "none";
const char* nvgLastError() {
	return g_nvg_last_error;
}
...
NVGcontext* nvgCreateInternal(NVGparams* params)
{
	...
	int i;
	if (ctx == NULL) {
		failure = "malloc(NVGcontext) failed";
		goto error;
	}
	...
	if (!ctx->commands) {
		failure = "malloc(commands) failed";
		goto error;
	}
	ctx->ncommands = 0;
	...
	if (ctx->params.renderCreate(ctx->params.userPtr) == 0) {
		failure = "backend renderCreate returned 0";
		goto error;
	}
	...
	if (ctx->fs == NULL) {
		failure = "fonsCreateInternal failed";
		goto error;
	}
	...
	if (ctx->fontImages[0] == 0) {
		failure = "renderCreateTexture(font atlas) returned 0";
		goto error;
	}
	ctx->fontImageIdx = 0;
	snprintf(g_nvg_last_error, sizeof(g_nvg_last_error), "%s", "none");

	return ctx;

error:
	snprintf(g_nvg_last_error, sizeof(g_nvg_last_error), "%s", failure);
	printf("nvgCreateInternal failed: %s\n", failure);
